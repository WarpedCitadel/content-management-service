...
COPY --from=builder /app/target/*.jar /content-management-service-0.0.1-SNAPSHOT.jar

RUN mkdir /deps && cd /deps && \
    jar -xf /content-management-service-0.0.1-SNAPSHOT.jar BOOT-INF/lib && \
    jdeps --ignore-missing-deps --print-module-deps --multi-release 25 \
    --class-path 'BOOT-INF/lib/*' \
    /content-management-service-0.0.1-SNAPSHOT.jar > /modules.txt
...
EXPOSE 8082
CMD ["java", "-jar", "/app/content-management-service-0.0.1-SNAPSHOT.jar"]