...
public class ProfileService {
    ...
    public String createGameProfile(GameProfileDto gameProfileDto) {
    ...
    public void updateGameProfile(GameProfileDto gameProfileDto) {
        GameProfileModel gameProfileModel = new GameProfileModel();
        gameProfileModel.setUserUUID(gameProfileDto.userUUID());
    ...
    public GameProfileDetailsDto getGameProfile(String gameProfileUUID) {
        GameProfileDetailsModel gameProfileModel = profileRepository.getGameProfile(gameProfileUUID);
        if (gameProfileModel == null) {
            throw new RuntimeException("Requested game profile ID: " + gameProfileUUID + " does not exist");
        }
    ...
    public void deleteGameProfile(DeleteGameProfileDto gameProfileDto) {
        ...
        log.info("Successfully deleted game profile ID: ({}) from user ID: ({}) in ({}) ms",
                gameProfileDto.gameProfileUUID(), gameProfileDto.userUUID(), elapsed);
    }
        ...
        try {
            ...
            do {
                ...
                for (S3Object s3Object : objectList.contents()) {
                    ...
                    log.info("Object key ({}) in ({}) bucket marked for DELETION", objectList.prefix(), bucketName);
                }
                if (!deletionList.isEmpty()) {
                    ...
                    log.info("Successfully deleted ({}) objects in ({}) bucket", deletionList.size(), bucketName);
                }
                continuationToken = objectList.nextContinuationToken();
            } while (continuationToken != null);
        } catch (S3Exception exception) {
            log.error("Failed to delete object key ({}) in ({}) bucket Reason: ({})",
                    prefix, bucketName, exception.toString());
            throw new RuntimeException("Failed to delete objects in storage");
        }
    }
        ...
        if (gameProfileDetailsModel.getGameFileDetailsModel().getFileName() != null) {
            try {
                ...
                log.error("Failed to generate file url for game profile ID: ({}) since file was ({})",
                        gameProfileDetailsModel.getGameProfileUUID(), gameProfileDetailsModel.getGameFileDetailsModel().getFileName().get(i));
    ...
    private String generateHtmlGameUrl(GameProfileDetailsModel gameProfileDetailsModel) {
        ...
        try {
            ...
            log.error("Failed to generate HTML5 url for game profile ID: ({}) since file was ({})",
                    gameProfileDetailsModel.getGameProfileUUID(), gameProfileDetailsModel.getGameFileDetailsModel().getFileName().get(i));
    ...
    private String findFilesByExtension(GameProfileDetailsModel gameProfileDetailsModel, int index) {
        ...
        try {
            ...
            log.error("Failed to find .html file for in file ({}) game profile ID: ({}) Reason: ({})",
                    gameProfileDetailsModel.getGameFileDetailsModel().getFileName().get(index),
                    gameProfileDetailsModel.getGameProfileUUID(), exception.toString());
